...
if [ $# -lt 4 ]; then
    ...
    echo "example: $0 linux wardf/meld [directory 1] [directory 2]"
    echo "OS Options:"
    echo -e "\tlinux"
    echo -e "\tosx"
    exit 1
...
if [ "X$1" == "Xosx" ]; then
    ...
    docker run --rm -e DISPLAY=$MYIP:0 --net=host -v $3:/former -v $4:/latter $2
fi

if [ "X$1" == "Xlinux" ]; then
    docker run --rm -e DISPLAY --net=host -v $3:/former -v $4:/latter -v $HOME/.Xauthority:/root/.Xauthority $2
fi
