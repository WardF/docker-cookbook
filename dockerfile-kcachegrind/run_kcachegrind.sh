#!/bin/bash
#
# Convienence script to launch a docker container version of kcachegrind with all of the appropriate arguments.
...
if [ $# -lt 2 ]; then
    ...
    echo "example: $0 linux wardf/kcachegrind"
    echo "OS Options:"
    echo -e "\tlinux"
    echo -e "\tosx"
    exit 1
...
if [ "X$1" == "Xosx" ]; then
    ...
    docker run --rm -v $(pwd):/workdir -e DISPLAY=$MYIP:0 --net=host $2
fi

if [ "X$1" == "Xlinux" ]; then
    docker run --rm -v $(pwd):/workdir -e DISPLAY --net=host -v $HOME/.Xauthority:/root/.Xauthority $2
fi
